...
if command -v npm >/dev/null 2>&1; then
    if [ -f "$repo_dir/extensions/pi/mcp-bridge/package.json" ] && [ ! -d "$repo_dir/extensions/pi/mcp-bridge/node_modules" ]; then
        printf 'Installing mcp-bridge extension dependencies...\n'
        (cd "$repo_dir/extensions/pi/mcp-bridge" && npm install --no-audit --no-fund --silent) &&
            printf 'Installed mcp-bridge extension dependencies.\n' ||
            printf 'Failed to install mcp-bridge extension dependencies. Run: (cd %s/extensions/pi/mcp-bridge && npm install)\n' "$repo_dir"
    fi
else
...
EOF
    ...
    if python3 - "$opencode_config" "$opencode_instruction" <<'PY'; then
import json
import pathlib
import sys
